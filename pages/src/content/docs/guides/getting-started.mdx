...
<CardGrid>
	<Card title="环境要求" icon="computer">
		<p><strong>Go 1.18+</strong>：编译和运行driver-box</p>
		<p><strong>Git</strong>：下载源码</p>
		<p><strong>SQLite3</strong>：数据存储（通常系统已预装）</p>
	</Card>
	<Card title="系统要求" icon="server">
		<p><strong>最低配置</strong>：128MB RAM</p>
		<p><strong>推荐配置</strong>：256MB RAM以上</p>
		<p><strong>支持平台</strong>：Linux、Windows、macOS</p>
	</Card>
	<Card title="网络要求" icon="globe">
		<p><strong>设备通信</strong>：Modbus、MQTT等协议端口</p>
		<p><strong>HTTP API</strong>：默认8081端口</p>
		<p><strong>数据导出</strong>：根据导出配置的端口</p>
	</Card>
</CardGrid>
...
<Aside type="danger" title="重要提醒">
  如果问题无法解决，请查看控制台日志的详细错误信息，或访问我们的[GitHub Issues](https://github.com/iBUILDING-X/driver-box/issues)寻求帮助。
</Aside>