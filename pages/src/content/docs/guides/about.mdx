...
插件是 driver-box 提供了一项开放性能力，如若内置的插件不满足需求，用户可参考《[Plugin开发](/driver-box/plugins/development/)》实现一款自定义插件并集成至 driver-box。
...
[《Export 开发》](/driver-box/exports/development/)。

### 资源库
资源库是 driver-box 框架提供的一种资源管理能力。通过持续沉淀和复用资源库中的已有资源，逐渐提升项目工程实施效率。

资源库存储于`res/library/`目录。
<FileTree>

    - driver-box
        - res
            - library
                - index.json    资源库索引文件，便于快速定位资源
                - driver/       设备层驱动库
                - mirror_tpl/   镜像模块库
                - model/        物模型库
                - protocol/     通信协议层驱动库
