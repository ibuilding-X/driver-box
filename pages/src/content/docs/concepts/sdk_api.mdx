...
// 注册新Plugin
myPlugin := &MyPlugin{}
driverbox.EnablePlugin("my-protocol", myPlugin)
...
// 获取基础Export模块(用于注册自定义 API)
baseExport := driverbox.BaseExport()
```

## 自定义 Export 开发

开发者可以通过实现 `Export` 接口来扩展数据Export功能。
...
func (m *MyExport) ExportTo(deviceData plugin.DeviceData) {
    ...
    for _, point := range deviceData.Values {
        fmt.Printf("设备 %s 点位 %s 值: %v\n",
            deviceData.ID, point.PointName, point.Value)
    }
}
...
### 错误处理
- 所有 API 调用都应该检查错误返回值
- 使用 `driverbox.Log()` 记录关键错误信息

### 资源管理
- 使用 `defer` 确保资源正确释放
- 定时任务在不使用时应调用 `Disable()` 释放

### 性能优化
- 优先使用设备影子 API 获取实时数据,减少主动读取
- 批量操作优于多次单个操作
- 合理设置点位上报模式(`realTime` vs `change`)

### 并发安全
- 设备影子 API 是并发安全的,可以在多个 goroutine 中调用
- 核心缓存的查询操作也是并发安全的

### 事件处理
- 在 Export 模块中实现 `OnEvent` 方法来响应系统事件
- 使用 `TriggerEvents` 触发自定义事件
