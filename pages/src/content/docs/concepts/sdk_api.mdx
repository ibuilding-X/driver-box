...
// 获取指定设备
device, ok := driverbox.CoreCache().GetDevice("device-001")
if ok {
    fmt.Printf("设备属性: %+v\n", device.Properties)
}

// 判断设备是否存在
if driverbox.CoreCache().HasDevice("device-001") {
    fmt.Println("设备存在")
}
```
...
```go
// 获取指定模型的所有点位
points, ok := driverbox.CoreCache().GetPoints("modbus-device")
if ok {
...
// 根据设备ID查询点位
point, ok := driverbox.CoreCache().GetPointByDevice("device-001", "temperature")
if ok {
    fmt.Printf("点位配置: %+v\n", point)
}

// 根据模型查询点位
point, ok := driverbox.CoreCache().GetPointByModel("modbus-device", "temperature")
if ok {
    fmt.Printf("点位配置: %+v\n", point)
}
```
...
// 获取指定模型
model, ok := driverbox.CoreCache().GetModel("modbus-device")
if ok {
    fmt.Printf("模型详情: %+v\n", model)
}
```
...
```go
// 更新设备属性
err := driverbox.CoreCache().UpdateDeviceProperty("device-001", "unitID", "2")
if err != nil {
    driverbox.Log().Error("Update device property failed", err)
}

// 更新设备描述
err := driverbox.CoreCache().UpdateDeviceDesc("device-001", "更新后的描述信息")
if err != nil {
    driverbox.Log().Error("Update device desc failed", err)
}

// 删除设备
driverbox.CoreCache().DeleteDevice("device-001")
...
```go
// 获取连接信息
pluginName, connection := driverbox.CoreCache().GetConnection("conn-001")
if connection != nil {
    fmt.Printf("连接所属插件: %s\n", pluginName)
}

// 删除连接(如果无设备使用)
err := driverbox.CoreCache().DeleteConnection("conn-001")
if err != nil {
...
err := driverbox.CoreCache().AddModel("modbus", newModel)
if err != nil {
    driverbox.Log().Error("Add model failed", err)
}

// 删除模型(需先删除关联设备)
err := driverbox.CoreCache().DeleteModel("old-model")
if err != nil {
...
// 判断设备是否存在
if driverbox.Shadow().HasDevice("device-001") {
    fmt.Println("设备已注册")
}
...
// 添加设备到影子
driverbox.Shadow().AddDevice("device-004", "modbus-device", 5*time.Minute)
...
```go
// 获取设备点位值
value, err := driverbox.Shadow().GetDevicePoint("device-001", "temperature")
if err == nil {
    fmt.Printf("温度: %v\n", value)
}

// 设置设备点位值
err = driverbox.Shadow().SetDevicePoint("device-001", "temperature", 25.6)
if err != nil {
    driverbox.Log().Error("Set device point failed", err)
}

// 获取设备所有点位
points, err := driverbox.Shadow().GetDevicePoints("device-001")
if err == nil {
...
// 获取点位详细信息(包含更新时间和写入值)
pointDetail, err := driverbox.Shadow().GetDevicePointDetails("device-001", "temperature")
if err == nil {
...
```go
// 获取设备最后更新时间
updatedAt, err := driverbox.Shadow().GetDeviceUpdateAt("device-001")
if err == nil {
    fmt.Printf("设备最后更新: %s\n", updatedAt)
}
```
